  ...
  return (
    <div className="space-y-0">
      {criteria.map((criterion) => (
        <div key={criterion.key} className="score-pill py-4 flex items-center gap-4">
          <span className="label-mono w-28 shrink-0">{criterion.label}</span>
          ...
          <div className="flex items-baseline gap-1 w-20 justify-end">
            <span className="score-display text-xl" style={{ color: getScoreHex(criterion.value) }}>
              {criterion.value.toFixed(1)}
            </span>
            <span className="text-xs text-ink-4">/10</span>
          </div>
        </div>
      ))}
    </div>
